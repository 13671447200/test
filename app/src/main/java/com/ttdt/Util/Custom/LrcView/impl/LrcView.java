package com.ttdt.Util.Custom.LrcView.impl;
...
import com.ttdt.Util.Custom.LrcView.ILrcView;
import com.ttdt.Util.Custom.LrcView.ILrcViewListener;
import com.ttdt.Util.DensityUtil;
 ...
 */
public class LrcView extends TextView implements ILrcView {
     ...
     * 歌词字体大小默认值
     **/
    private int mLrcFontSize = 18;    // font size of lrc
    /**
     * 歌词字体大小最小值
     **/
    private int mMinLrcFontSize = 14;
    /**
     * 歌词字体大小最大值
     **/
    ...
    public LrcView(Context context, AttributeSet attr) {
        ...
        mLrcFontSize = DensityUtil.dip2px(context,mLrcFontSize);
    }
...
//                float delta = ((currentPositon-timePoint)/sleepTime )*textHeight;
                //屏幕的的坐标 = 行高 + 移动的距离
                plush = mLrcFontSize + ((currentPositon-timePoint)/sleepTime )*mLrcFontSize;
               // Log.e(TAG,"plush " + plush + "," + "timePoint " + timePoint+"," + "sleepTime" + sleepTime);
            }
...
//        }
    ...
    public void seekLrc(int position, boolean cb) {
        ...
        if(mHignlightRow + 1 < mLrcRows.size()){
            sleepTime = mLrcRows.get(mHignlightRow+1).time - mLrcRows.get(mHignlightRow).time;
            timePoint = mLrcRows.get(mHignlightRow).time;
        }
